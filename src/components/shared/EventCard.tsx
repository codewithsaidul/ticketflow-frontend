...
export function EventCard({  title, slug, date, location, seatLayout, image, category }: IEvent) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg glass hover:border-primary/50 transition-all duration-300 h-full flex flex-col">
      ...
      <div className="relative h-48 sm:h-56 overflow-hidden">
        <Image
          src={image || "https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"}
          alt={title}
          fill
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-primary/10">
          ...
          <Button size="sm" className="rounded-full bg-primary hover:bg-primary/90 text-primary-foreground cursor-pointer">
            <Link href={`/events/${slug}`}>Book Now</Link>
          </Button>
        </div>
      </div>
    </div>
